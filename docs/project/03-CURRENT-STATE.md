...
آخر تحقق وظيفي: 2026-09-02 على `develop` عند baseline `3ad64fc` مع تغييرات Slice 3 غير ملتزمة.
...
- اكتملت Study Paths Slice 3 محليًا: migration registry بالحقول الخمسة الدنيا فقط، بلا وصول للمتصفح ومع `service_role` قراءة فقط، وقيود/trigger تمنع تبديل digest أو الهوية والحذف وتجعل retirement أحاديًا، وCLI إداري يقارن manifest Git بالـsnapshot المستضاف عبر أسرار البيئة. لم تطبق migration أو تختبر مستضافًا بعد.
...
- لا يوجد بعد مسار علمي منشور فعليًا أو واجهة تسجيل/تقدم Study Paths، ولا تطبيق مستضاف لـprojection الإصدارات، ولا تسجيلات طالب أو RPC للمسارات. الـfixture الحالي تقني بحالة `draft` وليس مسارًا علميًا منشورًا، وقوائم التشغيل تبقى للتصفح والتسلسل فقط.
...
راجع بوابة Study Paths Slice 3، ثم طبق migration registry وتحقق منها مستضافًا بأمان قبل بدء Slice 4 للتسجيلات؛ تبقى واجهة الطالب للشرائح اللاحقة، ثم Ask AI، من دون توسيع نطاق مساحة الطالب V1 ضمنيًا.
