...
### projection النشر في Supabase — منفذ في Slice 3V

`study_path_versions` ليس نسخة ثانية من المنهج. حقول projection فقط:
...
### تسجيل الطالب في Supabase — منفذ في Slice 4 و4.1
...
يعرض قسم «مساراتي» المنفذ في Slice 6 كل تسجيل `active` مستقلًا:
...
تعرض لوحة الطالب كذلك paused مع تقدمه دون Continue، وwithdrawn/superseded في سجل للقراءة فقط دون تقدم تاريخي مشتق على dashboard. رابط live بلا `?enrollment=`؛ رابط التاريخ بمعرّف التسجيل المملوك الدقيق. يبقى active المكتمل active بلا Continue. تعالج القراءة missing/corrupt states دون اختراع تقدم أو رابط أو اختيار فائز، ويستقل فرع المسارات عن فرع الدروس العامة في النجاح والفشل وإعادة المحاولة. تفاصيل الحدود والأجيال والقبول في خطة Slice 6.

## RLS وRPC — الحدود المنفذة في الشرائح السابقة
...
- [x] enroll controls للتعريفات المنشورة وصفحة enrollment مملوكة.
- [x] تقدم الوحدة/المسار وPath Continue مشتقة من التعريف static و`lesson_progress`.
- [x] loading/empty/error/retry وواجهة RTL متجاوبة؛ تحقق العرض حتمي، دون اختبار متصفحي authenticated جديد.
- [x] سجل تسجيلات مملوك وتحديد تاريخي مغلق عند الفشل، ومعاينة وترقية أمامية صريحة ومصالحة بلا replay ضمن Slice 5D. انظر `05d-study-path-history-upgrade.md`.

### Slice 6 — دمج «مساراتي» في مساحة الطالب

- [x] Slice 6A عند `5ab65c0`: قراءة owner-wide كاملة بالـcursor والتجميع المغلق عند الفشل وحل الإصدار المثبت واتحاد مفاتيح التقدم والاشتقاق النقي.
- [x] Slice 6B عند `bc04797`: بطاقات المسارات المختلفة live بالتوازي وتقدمها المثبت وContinue الخاص بكل active غير مكتمل؛ لا primary/focused path.
- [x] paused بتقدم ودون Continue، وwithdrawn/superseded بسجل قراءة وروابط تسجيل دقيقة، وحالات missing/corrupt آمنة.
- [x] الحفاظ منفصلين على Continue العام وRecent؛ فرعا قراءة مستقلان، مع تحقق getSession/getUser وحماية تغير الحساب والطلبات القديمة وإعادة المحاولة وfocus/pageshow وdispose.
