...
اكتملت Slice 1 من Study Paths V1 كنموذج مجال static ونقي في Git، واكتملت Slice 2 كطبقة قراءة عامة static، واكتملت Slice 3 محليًا بregistry نشر أدنى لم يطبق مستضافًا بعد. يدعم النموذج مسارًا بهوية دائمة وmetadata عرض وحالة و`currentVersion`، وإصدارات منهج ذات وحدات مرتبة وأهداف تعلم ودروس مطلوبة مرتبة تشير إلى `lesson_key` فقط. يوجد fixture تقني صغير بحالة `draft` للاختبار؛ ليس مسارًا علميًا منشورًا ولا يدخل الكتالوج العام.
...
- migration محلية لـ`study_path_versions` لا تخزن إلا `(path_id, version, definition_digest, published_at, retired_at)`، وتحجب browser roles وتمنح `service_role` قراءة فقط، وتمنع تغيير النسخة المنشورة أو حذفها وتسمح بتقاعد أحادي الاتجاه.
...
- لم تطبق migration `study_path_versions` على Supabase المستضاف ولم يجر hosted verification، ولا يوجد persistence للمنهج أو enrollments أو RPC خاصة بالمسارات.
...
بعد اكتمال registry النشر الأدنى محليًا، الخطوة التالية هي مراجعة migration وتطبيقها والتحقق منها مستضافًا بأمان، ثم التسجيل وواجهته وتقدم المسار الظاهر، وأخيرًا دمج «مساراتي» والتحقق التشغيلي. أدوات التحرير والتوصيات ليستا ضمن V1 الحالي.
