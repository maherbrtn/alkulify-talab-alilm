...
**Slice 7 operational closeout PASS ; Study Paths V1 implementation techniquement close ; first reviewed scientific path present in local public catalog ; hosted publication registry insertion and authenticated production browser acceptance pending.**

يضم `publicStudyPaths` محليًا أول مسار علمي مراجع: «شرح ثلاثة الأصول»، بحالة `published` وإصدار 1، من تعريف مستقل عن الـfixture. يبقى الـfixture التقني بحالة `draft` وخارج الكتالوج. لم يُدرج سجل نشر المسار في Supabase المستضاف، ولم يُنشر بناء الموقع أو يُجر القبول الإنتاجي المصادق عليه في هذه المهمة.
...
- فهرس `/study-paths/` وقالب current وقالب نسخة تاريخية، كلها من تعريفات Git فقط وبلا Auth أو Supabase. يعرض الفهرس المحلي الآن المسار المراجع وحده ويولد قالب current صفحة منهجه؛ لا توجد صفحة نسخة تاريخية للإصدار الحالي الوحيد.
...
## أول منهج علمي مراجع — متاح محليًا

- المصدر: `src/lib/study-path-definitions/sharh-thalathat-al-usul.json`؛ النصوص والتقسيم من مراجعة صاحب المشروع، ولا يُستخدم ASR نصًا علميًا معتمدًا.
- العنوان: «شرح ثلاثة الأصول»؛ الـslug: `sharh-thalathat-al-usul`؛ الحالة `published`؛ `currentVersion = 1`، وإصدار واحد فقط.
- `pathId`: `e17f26d2-2a38-40af-9984-2af8ec45ba43`؛ هوية جديدة مع أربع هويات وحدات UUID v4 جديدة، بلا إعادة استخدام لهويات الـfixture.
- `publishedAt`: `2026-09-14T16:36:21.235Z`؛ تاريخ نشر التعريف المحلي المثبت، وليس إثباتًا لإدخال مستضاف.
- SHA-256 للإصدار 1: `2374ced4629fcafdb1be830231e2eab647d3d7394f167453f68fe1252fc34afb`؛ يولده manifest القائم من التعريف canonical دون ملف manifest يدوي موازٍ.

| الوحدة | الدروس بالترتيب الإجمالي |
|---|---|
| المقدمات والمسائل التمهيدية | 1، 2، 3 |
| الحنيفية والعبادة ومعرفة الرب | 4، 5 |
| الأصل الثاني: معرفة دين الإسلام | 6 |
| الأصل الثالث: معرفة النبي ﷺ | 7 |

تستخدم عضوية المنهج مفاتيح `lesson_key` السبعة القائمة فقط. تحقق تطابقها مع الفيديوهات والعناوين المحددة قبل الإضافة، ويظل حل المصدر والروابط عبر resolver الحالي. لا تغيير في registry أو هوية الدروس أو المجال أو schema/migrations. تثبت selfchecks المسار الوحيد والنسخة والوحدات والترتيب والروابط والـdigest المثبت واستبعاد الـfixture واستقلال الكتالوج عن Auth/Supabase والتخزين والطلبات الشبكية. يقارن تحقق محلي manifest مع projection داخل الذاكرة، ويثبت أن registry فارغًا يعيد `missing`؛ هذا ليس تحققًا مستضافًا.
...
التحقق محلي وحتمي للقراءات/controller/rendering والسباقات والحالات الطرفية، مع بوابات build وCI للـcommits المثبتة؛ الأدلة في خطة Slice 6. لم تختبر رحلة My Paths إنتاجية فعلية في متصفح مصادق عليه؛ إضافة التعريف المراجع محليًا لا تستكمل هذا القبول ولا إدخال سجل النشر المستضاف.
...
- قبول المتصفح والإنتاج الحقيقي بمستخدم مصادق عليه: **PENDING**؛ أصبح التعريف المراجع متاحًا محليًا، وبقي إدخال سجل النشر المستضاف ونشر الموقع وإجراء الرحلة الفعلية.

## غير منفذ بعد

- إدخال سجل نشر «شرح ثلاثة الأصول» في المستضاف، والتحقق من مطابقته للـmanifest، ونشر بناء الموقع، ثم قبول رحلة التسجيل وMy Paths الإنتاجية في متصفح مصادق عليه. التعريف العام وصفحتا المنهج والطالب متاحان في البناء المحلي فقط؛ لا إثبات إنتاجيًا جديدًا.
...
1. اكتملت محليًا إضافة أول مسار علمي مراجع إلى الكتالوج؛ يبقى إدخال صف النشر المطابق في `study_path_versions` ثم تشغيل `pnpm verify:study-path-publications` إداريًا ونشر بناء الموقع. لا يُتاح enroll لهذا الإصدار مستضافًا قبل وجود صفه.
2. إجراء القبول المتصفحي الإنتاجي المصادق عليه لرحلة التسجيل وMy Paths والتقدم والاستئناف.

شريحة تحصين Supabase العامة أُغلقت لاحقًا ومستقلة عن هذه الإضافة؛ حالتها المؤكدة في `03-CURRENT-STATE.md`.
